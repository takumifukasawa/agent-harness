...
- 開始: 2026-09-20
- 状態: 進行中（フェーズ 1 / T01・T02 完了、次は T04 → T03）
...
| T04 | doctor B6 をフック実行不可で FAIL に上げる（決定 0007） | todo | T02 は WARN 据え置きにしたが、`check` が緑のままでは今回と同じ見落とし方が残るため FAIL に上げる。Windows（`core.filemode=false`）で偽 FAIL を出さない判定は T02 の形を保つ |
