...
| 8 | **`harness gc` に実行経路の検査が無い。** `harness check` の 15 検査にも `tests/*.sh` にも `gc.sh` を走らせる経路が無いため、`gc.sh:42` の `date -d`（GNU 専用。macOS では `illegal option`）が**壊れていても誰も気づかない**。T01 の実装役が「未到達なので直さない」と正しく判断した、その根拠がこの穴そのもの | gc / テスト | 2026-09-20 | 未着手（`date -d` を直すだけでなく `tests/gc.sh` を足して `checks.sh` に登録する。経路が無いまま直しても検証できない） |
