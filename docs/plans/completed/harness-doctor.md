...
- 2026-09-18: T09 done（Red f6a78de → Green fc0be95、決定 0004、tests/doctor.sh 27→34、再試行 0、約 34 分）。`source` を機械ローカルの上書きへ逃がした（`effective_source()`: 環境変数 `HARNESS_SOURCE` > `.harness/source.local` > `manifest.source`）。manifest は共有値（公開 URL）に固定。`status` はどの層から来たかを表示する。**統括が禁止条件として渡した「検査とテストがネットワークに触らないこと」を実測で確認し、シナリオとして自動化した**（上書きが無く manifest が URL の状態でも clone キャッシュが作られないことで判定）。途中の実害: Red 段階で upstream のシナリオが正本の `SKILL.md` に書き込んだ（旧実装が manifest の絶対パスを見るため）→ `git checkout` で復旧し、setup で到達不能 URL に固定した。
- 2026-09-18: T16 done（マージ 921b415）。決定 0004 に README / CHANGELOG / architecture を追随。**worktree が 25 コミット遅れで払い出されていたのを `git merge --ff-only main` で追いついてから着手**（T14 の学びを指示に入れた効果）。
- 2026-09-18: T10 done（Red c267b15 → Green 9087f10 → 検査登録 da2da39 → 同期 9cc575c、検査 10→12 件、doctor scenarios 34→38、再試行 0、約 42 分）。頑健性 5 件と検査・テスト枠の穴 8 件。**13 件中 2 件は T14 / T08 で既に解消済みだと突き止め、盲目的に直さず回帰テストだけ足した**（うち 1 件は manifest のパスにコマンド注入を仕込むカナリアテスト）。
- 2026-09-18: T17 done（983b22c → 9fe7336）。**VERSION 0.4.0**、CHANGELOG を `## [0.4.0] - 2026-09-18` へ、`docs/architecture.md` の不変条件 2 行を「強制済」へ（実在を確認してから書かせた）。
- 2026-09-18: **手順の穴が 1 件見つかった** — T10 は利用者に影響する変更（doctor の頑健性、検査 2 件の追加）を入れたのに CHANGELOG の `[Unreleased]` に書いておらず、版切りの T17 で取りこぼしかけた。実装役の指示テンプレ（`task-orchestrate` §2.1）に「利用者に影響する変更なら `[Unreleased]` に書く」を足す必要がある（`[harness候補]`）。

## 結果（2026-09-18 完了）

- **実装 5 タスク（T01〜T05）→ 最終レビュー（4 観点 + 反証 1 回）→ 修正 10 タスク**で完了。再試行 0、blocked 0。
- **最終レビューは緑の検査（9 件 / 21 シナリオ）を通り抜けた high 4 件を掘り出した**: 壊れた manifest で「異常なし / exit 0」を返す偽の全快（3 観点が独立に報告）、直し方が導入先に無いパスを案内、`update` が変更済み managed を直さない根本原因（副次的に `merge_agents_md` のデータ欠損を発見）、B3 の復旧経路が存在しない。
- **反証は 1 件だけ回して当たり**: 「`update` の仕様を変える」という高コストの偽陽性を潰し、実際は doctor の文言修正（低コスト）で済むと確定した。「単独報告かつ修正コスト高だけに回す」という縛りは元が取れる。
- **ユーザーの問いが実害を先取りした**: 「そもそも `source` はローカルパスでいいのか」という問いから T09 の設計変更が決まり、その後 T14 が worktree で実際に事故（main tree の未コミットを取り込みかけた）を踏んだ。
- 最終状態: 検査 12 件 pass（doctor 38 シナリオ / update 13 シナリオ）、`harness doctor` は OK=16 WARN=1（jq 不在）FAIL=0、drift なし、VERSION 0.4.0。

## 数字（次に見積もるとき用）

| | 実測 |
|---|---|
| 1 タスクのサイクル | 実装 13〜60 分（中央値 約 40 分）。統括の判定込みで +10 分 |
| モデル別 | T06(opus) 39 分 / 40 往復 / 116k、T07(sonnet) 45 分 / **344 往復** / **462k**。下位モデルはコストが下がるが速くはならない |
| 検査 | 21 シナリオで 4m41s → T13 後 2m10s → 38 シナリオで 6〜8 分。`harness init` 1 回 = 11.25 秒が単価 |
| 並列化 | 成立したのは 3 ペアだけ（T13∥T08 / T10∥T16 / T14∥T15）。他は `doctor.sh` / `tests/doctor.sh` / `bin/harness` に集中して分離できない |
| レビュー | 4 観点 + 反証 1 = 5 体、約 53 万トークン。opus 4 体並列でレート上限に当たり 3 体が停止した |
