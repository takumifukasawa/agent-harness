...
├── scripts/             # check.sh（検査ランナー、LLM 不使用）、doctor.sh（診断）、gc.sh（docs の腐敗検知）、seed-case.sh（配布先の case 衝突判定。seed（#13）と managed（#14）の両方が対象、doctor.sh と bin/harness が共有）、session-start.sh
