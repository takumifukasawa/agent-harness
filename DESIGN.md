...
> 状態: **0.8.0（2026-09-23）**。①の雛形・`task-orchestrate`・②の配管はすべて実装済みで、このリポジトリ自身に導入済み。**`task-orchestrate` で 7 題材を通した**が、**うち 6 題材は②の配管だった**（[決定 0010](docs/decisions/0010-stop-building-plumbing.md) で打ち止めにした）。**§11 の ⬜ はすべて埋まった** —— 2026-09-23 に実プロジェクト `aesthetic-comparison` で 1 題材を §5 で通し、確かめることの 4 項目に実測が付いた（§11 の小節）。
...
| ✅ | **① dogfood**（2026-09-23 完了）: 実プロジェクト `aesthetic-comparison`（Next.js）で題材 1 つを §5 で通した（`readable-page`: 1 行 1469 コードポイントの詰め込みを解体し、再発を eslint の `max-len` で塞ぐ。3 タスク + 再試行 1 回 + 最終レビュー 2 観点）。確かめたこと 4 項目の結果は下の小節 |
...
### dogfood の実測（2026-09-23）

**2 系統ある**: このリポジトリ自身で **7 題材・約 30 タスク**（対象はハーネス自身）と、実プロジェクト `aesthetic-comparison` で **1 題材 3 タスク**（`readable-page`。ハーネスを*使う*側）。後者が §11 の dogfood 項目の本体で、[決定 0010](docs/decisions/0010-stop-building-plumbing.md) で②の配管づくりを打ち切ってから着手した。

**1. 再試行「新しい 1 体」の精度とコスト** — **再試行は 6 題材で 0 回**。1 タスク 1 体で、spec の節・受け入れ条件・`files_scope`・規律の本文を渡せば一発で通っている。コストは **1 タスク 6〜16 分**（下位モデル）。ただしこれは**統括が指示を作り込んだ結果**であって、「新しい 1 体」方式の精度そのものの証拠ではない。指示が薄いときにどうなるかは未検証。

**2. Codex でのパス限定規律** — `cross-env` の T08 で Codex の実装役を 1 周回した。**規律は統括が本文を貼る方式で機能した。**Codex はルートから cwd までの連鎖しか読まないため、パスだけ渡しても載らない（`docs/rules/README.md` に記載）。

**3. `checks.sh` に何を登録すると効くか** — 2 プロジェクトの実測:

| | 効いた検査 | 理由 |
|---|---|---|
| agent-harness（22 件） | `installed copies in sync` | 正本と導入コピーのずれを**毎回**捕まえる。dogfood 構造では最も頻繁に落ちる |
| | `gc scenarios` | **検査自身の回帰**。検査を足す題材では、これが無いと成果が空洞になる |
| | `bash 3.2: 禁止構文` | 環境差を**構文レベル**で塞ぐ。実行環境を用意せずに防げる |
| aesthetic-comparison（4 件） | `typecheck`（fast） | **`fast` の判断は実測で決めた**: typecheck 3s → fast、lint 13s / build 6s → 非 fast。「型エラーのあるコミットを止める価値は 3 秒の待ちに見合う」 |
| | `deps installed`（fast） | 依存未インストールだと 3 つとも同じ理由で落ちるので、**最初に 1 件だけ見て案内する** |

**効かなかったもの**: seed の `docs index exists` は一度も落ちていない（存在確認だけなので、腐りは捕まえられない）。

**4. `task-orchestrate` の手順で統括が迷う箇所** — 実際に迷った / 間違えたもの:

- **レビューの観点**: 既定の 4 観点（仕様突合 / 並行性 / 認可 / 機能の完結性）が題材に合わず、**3 題材連続で差し替えた**。成果物が bash と文章のプロジェクトでは「並行性 / 認可」が空振りする。**観点は題材ごとに選び直すものと考えた方がよい**
- **タスクの並列可否**: 「触るファイルが重なるか」で判断して事故った（`update` が `files_scope` を超えて正本全体を同期し、他タスクの未コミット変更を巻き込んだ）。正しくは**「全体同期コマンドを含むか」**（`docs/learnings.md` 2026-09-21）
- **正常な途中状態の扱い**: 「レビュー中」「ユーザー承認待ち」が `gc` に書き戻し忘れと誤検知された。**2 回とも検査側を直した**が、`task-orchestrate` が通る状態を検査が知らないという非対称は残る
- **セッションを切る原則**: §5 の「統括のセッションを切らない」は**解釈**だと本文に明記してあるが、実際には §2.3「タスクが完了したら切る」から **3 回続けて外れた**（2026-09-22〜23 は 7 タスク + レビュー 5 体を 1 セッションで通した）。**3 回守れない原則は、原則の方が実態に合っていない。**セッションを切る判断基準そのものを見直す必要がある

### 実プロジェクトで 1 題材通して分かったこと（`readable-page`、2026-09-23）

**効いたもの**:

- **「検査が効くことを証明せよ」という受け入れ条件が、2 回とも検査の空洞を暴いた。** 題材は「1 行への詰め込みの再発を `eslint` の `max-len` で塞ぐ」もので、受け入れ条件に「**整形前のコードを lint にかけて実際にエラーが出ることを確認し、出力を report に貼る**」を入れていた。結果は **1 回目 0 件 → 2 回目 2 件 → 3 回目 19/19 件**。1 回目・2 回目は `npm run lint` 自体は緑で、この条件が無ければ「検査を入れた」つもりで**再発を防げない設定が残っていた**。`AGENTS.md` の「検査は回避手段も塞ぐ」を、受け入れ条件の側から担保した形。
- **実装役が 3 回とも spec の誤りを実測付きで報告した。** `ignoreStrings` を必須にした誤り、`ignorePattern` に頼った誤り、文字数をバイトで数えた誤り（`awk` の `length()` はロケールによってバイト数を返す）。**いずれも統括が書いた spec が間違っていた。**実装役は指示どおり作ったうえで「これでは目的を達成できない」を `questions` に返しており、`docs/roles/implementer.md` の「質問はしない／疑問は questions に書いて返す」が、**指示に従いつつ異議を記録に残す**形で機能した。
- **レビュアーが統括の手順違反を見つけた。** `docs/plans/active/<slug>.md` を作らず、決定を `docs/decisions/` ではなく spec 本文に直書きしていた件（統括が `task-orchestrate` §5 を誤って引用した）。**統括を縛るものが無いという非対称**は [writeback-sensors](docs/spec/writeback-sensors.md) で検査に落としたが、**手順そのものの誤読は今も人間かレビュアーしか見つけられない。**

**想定外だったもの**:

- **別リポジトリを操作すると、そのプロジェクトの `AGENTS.md` が統括のセッションに載らない。** `docs/rules/README.md` は「Codex は cwd 基準でしか読まないので統括が貼る」と書いているが、**Claude Code でも cwd の外のリポジトリなら同じ**。今回は規律を実装役の指示本文に貼って渡した。**「別リポジトリで作業するときは規律を貼る」はエージェントを問わない規律。**
- **`task-orchestrate` の「各段階でコミット」と、プロジェクト側の「コミットは都度ユーザーの許可」が衝突する。** 後者は実プロジェクトでは珍しくない。題材限定の例外を `AGENTS.md` に明記し、完了とともに撤去する形で解決した（コミットしない選択は、TDD の痕跡が残らず実装役が途中で止まると作業が消えるので採らなかった）。**ハーネスはこの衝突を想定していない。**

**コストの実測**: 実装役 1 タスクあたり **9〜24 分**（下位モデル）。このリポジトリでの 6〜16 分より長いのは、`browser-check`（実ブラウザで 3 画面サイズ × 外部画像取得）を前後で回す工程を含むため。再試行は **7 題材で 1 回だけ**（`readable-page` の T02。原因は spec の誤りで、実装役の失敗ではない）。
