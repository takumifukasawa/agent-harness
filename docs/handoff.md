...
1. **⬜3 dogfood の残り: ハーネスを*使う*普通のプロジェクトで機能 1 つを `task-orchestrate` で通す。** 「確かめること」4 項目のうち **3 項目は 2026-09-23 に `DESIGN.md` §11 の小節へ書き戻した**（再試行 0 回 / Codex は本文貼りで機能 / `checks.sh` に何が効いたか）。
   - **実プロジェクト `/Users/fukasawa-takumi/Documents/developer/aesthetic-comparison`（Next.js）の状態**（2026-09-23 に実機確認）: `harness init` **コミット済み**（`2f4f049`）。`.harness/checks.sh` は **4 件登録済み**（`deps installed` / `lint` / `typecheck` / `build`。`fast` の付け方は実測 typecheck 3s・lint 13s・build 6s で判断してある）。**作業ツリーはクリーン**
   - **残っているのはここ**: 向こうの `docs/plans/` と `docs/spec/` は **README だけで、計画も spec も 1 件も無い**。つまり**導入と検査登録までで止まっており、題材を 1 つ通してはいない**
   - **題材はユーザーが決める**（向こうのプロダクトに何を作るかの話なので）。向こうの handoff によれば、次は「説明の深さと作品の見やすさを調整する段階」
