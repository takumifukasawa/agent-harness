...
## 2026-09-22 統括が 1 セッションで全タスクを通すと、ハーネスが解こうとした問題をそのまま再現する [harness候補]
- 症状: `task-orchestrate` で 2 題材（12 タスク + レビュー 8 観点 + 反証 2 体）を**ひとつの統括セッションで通した**。後半、`DESIGN.md` を読まずに「このハーネスは手順を守らせる道具ではない」と総括して外し、ユーザーに指摘されて訂正した。統括の書き戻し漏れも計 5 件レビューに拾われた。
- 原因: `task-orchestrate` §2.3 は「統括のセッションは**再試行の間は切らない**が、**タスクが完了したら切る**（次のタスクに文脈を持ち越さないため）」と書いている。これを守らず通した結果、**`DESIGN.md` §1 が挙げる症状「1 機能が 1 つのコンテキストに収まらず、後半ほど仕様やコードを読む余地が減る」を、ハーネス自身の開発で再現**した。役割定義が禁じる「spec 本文とレビュー全文を文脈に載せない」も破っていた。
- 対処 / 再発したら: **タスクが done になったら、統括のセッションも畳む**（`session-handoff` → `/clear` → 次のセッションで `session-catchup`）。`.harness/state/` は「切っても続きから拾える」ための土台で、**切らずに済ませるためのものではない**。切らないのは再試行の間と準備フェーズだけ。「まだ入るから続けられる」は理由にならない（入ることと、精度が保たれることは別）。

## 2026-09-21 `harness update` はタスクの files_scope を超えて全体を同期する（並列タスクの事故） [harness候補]
- 症状: 実装役 2 体を「触るファイルが重ならないから」と判断して並列に走らせたところ、片方が `bash bin/harness update` を回した瞬間に、**もう片方の未コミットの `bin/harness` を導入コピー（`.harness/bin/harness` と `manifest.json`）へ巻き込んだ**。巻き込まれた側が `git checkout` と manifest の手で修復するはめになった。
- 原因: `update` は正本（`harness/` と `bin/harness`）**全体**を見て導入コピーを同期する。**タスクの `files_scope` は実装役の編集範囲を縛るだけで、`update` が同期する範囲には効かない。** 同じ作業ツリーを共有している以上、片方の未コミット変更も「正本の現在の内容」として同期対象になる。
- 対処 / 再発したら: **`bash bin/harness update` を回すタスクは並列にしない。** どうしても並列にするなら `git worktree` で作業ツリーを分ける。統括が並列可否を判断するときは「触るファイルが重なるか」ではなく「**リポジトリ全体を同期するコマンド（`update`）を含むか**」で見る。`files_scope` が分かれていても安全とは限らない。
...
- 原因: LLM は「その内容がコンテキストのどこに由来するか」を正しく申告できない。質問が指す対象（「agent-harness が管理しているブロックの、実装後に回すコマンド」）を特定できなければ、**読めていても「無い」と答える**。否定の回答は「読めていない」の証拠にならない。
- 対処 / 再発したら: **一意な canary 文字列を対象ファイルに仕込んで、それが返るかで判定する**（`ROOT-CANARY-5513` を追記して聞き直したら一発で返ってきた）。hook のように出力が混ざる対象は、**hook 自身にファイルを書かせて副作用で証拠を残す**（`date > .codex/ran-<event>.txt`）。ラベル（`hook: PreToolUse`）だけでは、個人設定の hook が走ったのかプロジェクトの hook が走ったのか区別できない。
...
## 2026-09-20 フックが走っていない気がしたら git index の mode を見る [harness候補]
- 症状: `core.hooksPath` は設定済みで `.githooks/pre-commit` も作業ツリーに存在するのに、コミット時に検査が動かない。`doctor` は `OK git hooks` と報告する。
- 原因: git が index に記録するのは**実行ビットの有無だけ**。index 上の mode が 100644 だと clone したツリーに実行ビットが付かず、git はフックを `hint:` 1 行だけ出して無視する（`hint: the '.githooks/pre-commit' hook was ignored because it's not set as executable`）。`harness init` の `chmod +x` は init したツリーにしか効かない。Windows の Git は `core.filemode=false` が既定なので、この差が開発機では表に出ない。
- 対処 / 再発したら: **まず `git ls-files -s .githooks/pre-commit` を見る**。100644 なら `chmod +x` に加えて `git update-index --chmod=+x` で index を 100755 にする。作業ツリーの `-x` を見る検査は `core.filemode != false` のときだけ有効にしないと Windows で偽警告になる。検査は `tests/githooks.sh`。フックが「走っているか」を確かめるには、検査をわざと壊して commit し、**拒否されて HEAD が動かない**ことまで見る（存在確認では門番になっていない）。

## 2026-09-20 `$( )` の中で呼ぶシェル関数はグローバル変数を書けない [harness候補]
- 症状: 集計用のカウンタを関数の中で増やしているのに、呼び出し元では常に初期値のまま。
- 原因: コマンド置換 `$( )` は**サブシェル**なので、その中で走る関数の代入は親プロセスに戻らない。出力を受け取る形で関数を呼ぶと、副作用だけが静かに消える。
- 対処 / 再発したら: 集計値が「なぜか初期値のまま」なら、その関数を `$( )` の中で呼んでいないか見る。集計は一時ファイルへの追記と `wc -l` に逃がすか、関数の結果を標準出力で返して呼び出し元で加算する。

## 2026-09-21 実 `$PATH` 全体を対象にしたスタブ化テストはタイムアウトする [harness候補]
- 症状: 「あるコマンドだけ無い環境」を作るテストで、`$PATH` 上の実行ファイルを丸ごとスタブディレクトリへ複製する経路を通すと、テストが `exit 137`（強制終了）で落ちる。
- 原因: 実 `$PATH` には**数千のファイルと GB 級のバイナリ**がある。`ln -s` なら一瞬だが、symlink が作れずコピーにフォールバックすると実体を複製しにいく。さらに `/usr/bin/sudo` のような setuid・所有者以外読み取り不可のファイルでは `cp` が Permission denied になる。
- 対処 / 再発したら: スタブ化のロジックを関数（`build_exe_stub(<src>,<dest>)` 等）に切り出し、**テストは小さな使い捨てディレクトリに対して直接その関数を呼ぶ**。実 `$PATH` を丸ごと壊す形のテストは書かない。個別ファイルの複製失敗で全体を落とすのも過検知になる（`sudo` のような無関係なファイルで落ちる）ので、**1 件も作れなかったときだけ失敗**させる。
