...
最終更新: 2026-09-20（題材 cross-env のフェーズ 1 / T01・T02 完了。残るは T03（版上げ）と人間の判断 1 件）

## いま何をしているか（1〜3 行）

題材は **cross-env**（エージェントと OS を問わず同じに動く）。**準備フェーズは完了し、合意はすべて spec と決定 0006 に書き戻してある。** いま反復フェーズで、**3 タスク中 2 つ（T01・T02）が done**。残るは T03（導入コピー同期・CHANGELOG・版上げ）。計画と進捗は `docs/plans/active/cross-env.md`、機械可読な状態は `.harness/state/`。
...
0. **先に人間の判断が要る（T03 の CHANGELOG の文面に効く）**: `doctor` の B6 が報告するフックの実行ビットを **WARN のままにするか、FAIL に上げるか**。また配布 seed（`harness/checks.seed.sh`）に実行ビット検査を入れるか。T02 は「FAIL にすると既存プロジェクトの `doctor: FAIL 0` 検査が突然赤くなる」ことを理由に **WARN 据え置き**にした。seed に入れなかったのは「init 直後はフックが未追跡で、index を見る検査が素通りになる」ため（`progress.json` の `open_questions`）。
1. **T03 を実装役に渡す**（`stages.json` の T03 をそのまま指示にする）。導入コピー同期・CHANGELOG・版上げ。**managed の移動もマーカー形式の変更も無いので minor（0.6.0）**。CHANGELOG には既存プロジェクト向けの手順（`git ls-files -s .githooks/pre-commit` が 100644 なら `chmod +x && git update-index --chmod=+x`）が T02 で既に入っている。
2. **T03 の後に main へ載せ、tech-debt #4 を閉じる**: `bash bin/harness init --source https://github.com/takumifukasawa/agent-harness.git` を 1 回回して `doctor` が FAIL 0 になることを確認する。**今落ちているのは公開 main が T01 前（305d57b）だからで、コード側の欠陥ではない**（現ブランチ内容の clone 経路では通る）。
...
- **`doctor` B6 のフック実行ビット指摘を WARN のままにするか、FAIL に上げるか。** 併せて配布 seed（`harness/checks.seed.sh`）に実行ビット検査を入れるか。T02 の判断は「WARN 据え置き・seed には入れない」。理由は上の NEXT 0 を参照。**T03 の CHANGELOG の文面に効くので、T03 の前に決めたい。**
- 準備フェーズの 3 件はすべて合意済み（spec の「合意済みの決定」と決定 0006）。
