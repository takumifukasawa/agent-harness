...
| ⬜ 3 | **① dogfood**: 自分のプロジェクト 1 つで機能 1 つを §5 で通す。**2026-09-23 時点で「確かめること」4 項目のうち 3 項目は実測が取れた（下の小節）。残るのは「ハーネスを*使う*普通のプロジェクトで機能 1 つを通す」こと** —— このリポジトリ自身で 6 題材を回したが、対象はすべてハーネス自身（うち 5 題材が②の配管）だった |
...
### dogfood の実測（2026-09-23 時点）

このリポジトリ自身で `task-orchestrate` を **6 題材・約 25 タスク**回した結果。**対象がハーネス自身である点が最大の限界**（[決定 0010](docs/decisions/0010-stop-building-plumbing.md)）。
