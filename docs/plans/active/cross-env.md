...
| T08 | `task-orchestrate` の 1 タスクを Codex で最後まで回す（B3） | **done** | 実装役の起動 → 戻り値 → 統括の 3 点判定まで。題材は小さい実タスク（未着手の負債 #2 や #7 が候補） |
| T09 | 標準 deny を Codex でどう効かせるか決めて `docs/decisions/` に残す（B4） | **done** | 候補は (a) `config.toml` の `sandbox_mode` / approval (b) **`PermissionRequest` / `PreToolUse` hook**（実機で形式を確認済み）(c) 現状どおり `.githooks/` だけで塞ぐ。**「どれが効くか」は実機で確かめてから決める**（この題材で 4 回続けて、推定のまま決めると外している） |
