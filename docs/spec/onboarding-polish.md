...
状態: **完了（2026-09-21、ユーザー承認済み）**。A / B / C をすべて満たした。最終レビュー（観点 4 つ）の指摘 10 件も処理済みで、未解決は無し。**残った限界は負債として起票してある**（#14: 検出は seed のみで managed は対象外 / #15: `hash_server_start` がエラーを恒久的に握りつぶす）。
